...
const sequelize = new sequelize_1.Sequelize(process.env.DB_NAME, process.env.DB_USERNAME, process.env.DB_PASSWORD, {
    ...
    logging: (sql, timing) => {
        if (process.env.NODE_ENV === 'development' || process.env.LOG_SQL === 'true') {
            logger_config_1.dbLogger.debug('SQL Query executada', {
...
sequelize.authenticate()
    .then(() => {
    logger_config_1.dbLogger.info('Conexão com banco de dados estabelecida com sucesso', {
    ...
    .catch((error) => {
    logger_config_1.dbLogger.error('Erro ao conectar com banco de dados', {
        error: error.message,
        database: process.env.DB_NAME,
        host: process.env.DB_HOST
    });
